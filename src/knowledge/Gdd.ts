...
import { Location, GDDAlgorithm } from '../state/State';
...
export function calc_gdd_total(
  ...
  if (itemsLocation.weather === undefined) return undefined;
  ...
  const daily_gdds_arr = daily_gdds_filter.map(item_2 => calcGdd(item_2.mintemp, item_2.maxtemp, item.base_temp, algorithm)
  );
